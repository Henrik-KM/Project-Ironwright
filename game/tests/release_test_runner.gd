...
func _test_localization(world: IronwrightReleaseWorld3D) -> void:
    ...
    _expect(service.text("story.record.endgame_severance.name") == "Die abgetrennte Wurzel" and service.text("notification.first_victory_achieved", ["Trennung"]) == "ERSTER SIEG · Trennung", "German endgame machine-report events must resolve stable localized record and victory text.")
...
func _test_presentation_review(world: IronwrightReleaseWorld3D) -> void:
    ...
    _expect(cathedral_review_offset.z <= 16.2 and cathedral_review_offset.y <= 9.8 and is_zero_approx(cathedral_review_offset.x), "Cathedral Quarter presentation review must use a centered focused frame for its nave, tower and choir hardware.")
