...
func _test_vertical_slice_presentation() -> void:
    ...
    _expect(world.camera.fov <= 45.0, "The representative tactical frame should use the tighter cinematic camera.")
    _expect(world.camera_height >= 11.0 and world.camera_height <= 11.4 and world.camera_distance >= 8.2 and world.camera_distance <= 8.6 and world.camera.fov <= 44.0, "The opening camera must keep the authored actors in the visual foreground beside the Heartforge without losing the route context.")
    ...
    if slice_root != null:
        ...
        _expect(slice_root.find_child("HighDefinitionStoryProps", true, false) != null, "The opening environmental story props must carry a bounded high-definition detail layer.")
        _expect(slice_root.find_child("EvacuationCase00", true, false) != null and slice_root.find_child("CaseLatchPanel00", true, false) != null, "The evacuation point must expose layered case and latch detail.")
        _expect(slice_root.find_child("DroneServicePanel", true, false) != null and slice_root.find_child("DroneStatusLens", true, false) != null, "The dead municipal drone must expose readable service and status detail.")
