...
func _run_all() -> void:
    ...
    if encounter_dressing != null and region_director != null:
        ...
        for raw_region_id in region_director.region_data.keys():
            ...
            if landmark.region_kind == &"nest":
                ...
                _expect(cathedral_release_detail != null and cathedral_release_detail.find_child("CathedralReleaseRoofline", true, false) != null and cathedral_release_detail.find_child("CathedralReleaseRoofEaveL", true, false) != null and cathedral_release_detail.find_child("CathedralReleaseClerestory02", true, false) != null and cathedral_release_detail.find_child("CathedralReleaseRoofFinial", true, false) != null, "Cathedral Quarter release dressing must expose a broken roofline, clerestory and finial silhouette.")
            ...
            if landmark.region_kind == &"greenhouse":
                ...
                _expect(landmark.find_child("GlasshouseFoundationPlinth", true, false) != null and landmark.find_child("GlasshouseFoundationRailFront", true, false) != null and landmark.find_child("GlasshouseFoundationRailRear", true, false) != null, "Municipal Glasshouse must expose a grounded foundation plinth and readable perimeter service rails beneath its glazed frame.")
    ...
    for index in outpost_samples.size():
        ...
        _expect(shelter_canopy != null and shelter_canopy_lens != null and shelter_canopy_rear_lens != null and shelter_canopy.get_parent().name == "RoofPlate" and shelter_canopy_lens.get_parent().name == "RoofPlate" and shelter_canopy_rear_lens.get_parent().name == "RoofPlate" and _mesh_vertex_count(shelter_canopy) > 24, "The authored outpost shelter must retain a dense parented raised canopy and bidirectional service lenses for a readable non-flat roof silhouette.")
