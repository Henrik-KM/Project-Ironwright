...
func _set_presentation_review_stage_for_page(is_region_page: bool) -> void:
	...
	# West Grid's broad steel hall and ceramic pressure tanks, plus East
	# Tenements' broad brick blocks, need a lower presentation key so authored
	# surfaces do not bloom into pale blocks at the compact exact-export review
	# size. Runtime lighting is unchanged; region accents remain untouched.
	var compact_region_light_scale := 0.58 if presentation_review_page == 4 else (0.62 if presentation_review_page == 5 else (0.72 if presentation_review_page == 12 else (0.68 if presentation_review_page == 11 else 1.0)))
