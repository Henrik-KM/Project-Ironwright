...
func _update_regions() -> void:
    ...
    for raw_region_id in region_director.region_data:
        ...
        if region_id == &"region.heartforge_district":
            drift += 0.0012
        landmark.set_pressure(maxf(float(data.get("base_pressure", 0.4)) * 0.72, landmark.pressure + drift))
...
func record_disturbance(position: Vector3, intensity: float, source_kind: StringName) -> void:
    ...
    var amount := clampf(intensity * 0.035, 0.005, 0.12)
...
func _species_for_region(region_id: StringName, selector: int) -> StringName:
    ...
    match kind:
        &"industrial":
            return &"burrower" if selector % 3 == 0 else &"razorhound"
        &"commercial":
            return &"sporecaster" if selector % 3 == 0 else &"skitterling"
        &"nest":
            return &"broodmass" if selector % 5 == 0 else (&"sporecaster" if selector % 2 == 0 else &"razorhound")
        &"research":
            return &"burrower" if selector % 2 == 0 else &"veilstalker"
        &"endgame":
            return &"broodmass" if selector % 4 == 0 else &"veilstalker"
        _:
