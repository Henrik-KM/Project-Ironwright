...
The director also enforces bounded active and medium actor budgets, assigning the nearest actors first. Medium actors retain their targets and state but advance on a coarse cadence; reduced actors use deterministic coarse movement and combat ticks. Release actors outside the active visual tier use a lightweight silhouette proxy, and may defer construction of their authored high-definition shell until active promotion, so high-definition geometry does not multiply routine spawn or rendering cost. Physical collision and simulation state remain present while the authored presentation shell is deferred.
