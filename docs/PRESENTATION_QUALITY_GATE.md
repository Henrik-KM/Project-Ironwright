...
6. cohesive urban modular kit with damaged facades, interiors, street furniture and vegetation — opening route set-piece pass implemented, every remote landmark now has a bounded presentation-only district apron, and Riverworks, Cathedral Quarter, Observatory Ridge and Root Cistern now have authored landmark shells; broader district breadth still required;
7. final lighting/material pass for the opening district;
8. human visual acceptance of representative opening gameplay;
9. repeat the accepted bar across mid- and late-game regions — representative West Grid, Riverworks, Cathedral Quarter and Root Cistern live review now passes for the reviewed approach frames; Observatory Ridge has a new authored shell awaiting live acceptance, and the remaining region families still require review;
