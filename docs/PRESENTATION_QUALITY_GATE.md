...
4. one production-quality Warden/Scrapper/Pathfinder family pass — the Warden now has an authored high-definition guardian shell with protected autocannon and heat-exchanger language, and the Scrapper now has an authored cargo/tool shell with salvage hardware; Pathfinder retains the shared beveled procedural vertical-slice pass with progression-aware level 2/3 frame finishes, with further authored refinement expected;
