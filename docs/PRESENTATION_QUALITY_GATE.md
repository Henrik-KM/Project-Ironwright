...
generated by the shared mesh builder. This keeps the guardian's weathered PBR
surface relief available after regeneration instead of silently flattening the
material attributes at export. The imported asset retains its existing cannon,
targeting and protection sockets; native aesthetic coverage verifies the
channels and the exact asset re-imported successfully under Dummy audio with
`--quiet-audio`.
