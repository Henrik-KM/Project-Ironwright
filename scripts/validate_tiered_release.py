#!/usr/bin/env python3
"""Validate Project Ironwright's tiered ecological escalation build."""
...
def validate_tier_configuration() -> None:
    ...
    if float(simulation.get("spawn_credit_cap", 99.0)) > 3.0:
        raise legacy.ValidationError("Spawn credit must remain bounded at three or fewer organisms")
    if int(tiers["1"].get("unit_cap", 0)) != 100:
        raise legacy.ValidationError("Prototype Tier-1 cap must remain 100")
    ...
    if tiers["1"].get("behaviour_profile") != "feral":
        raise legacy.ValidationError("Tier 1 must use the feral behavior profile")
    if set(tiers["1"].get("species_weights", {})) != {"skitterling"}:
        raise legacy.ValidationError("Tier 1 must contain only the primitive Skitterling prototype population")
    supported_tiers = set()
    for profile in nests.values():
        if isinstance(profile, dict):
            supported_tiers.update(int(value) for value in profile.get("supported_tiers", []))
    if supported_tiers != {1, 2, 3, 4, 5}:
...
def validate_species_mapping() -> None:
    ...
    if data.get("tier_system") != "population_driven_enemy_escalation":
        raise legacy.ValidationError("Enemy archetype registry must name the tier progression system")
    ...
    if len(tier_one) != 1 or tier_one[0].get("runtime_species") != "skitterling":
        raise legacy.ValidationError("Tier 1 must be a single numerous primitive species during the prototype balance pass")
...
def validate_tier_runtime_contracts() -> None:
...
def validate_dynamic_event_modifiers() -> None:
    data = load_json("game/data/enemy_tier_event_modifiers.json")
    operations = data.get("operations", {})
    technologies = data.get("technologies", {})
    endgame = data.get("endgame", {})
    if len(operations) < 10:
        raise legacy.ValidationError("Most major physical operations must define an ecological consequence")
    if not any(
        any(float(value) < 0.0 for value in effect.get("replenishment_delta_per_minute", {}).values())
        for effect in operations.values()
        if isinstance(effect, dict)
    ):
        raise legacy.ValidationError("At least one nest/suppression operation must reduce replenishment")
    if not any(
        any(float(value) > 0.0 for value in effect.get("replenishment_delta_per_minute", {}).values())
        for effect in operations.values()
        if isinstance(effect, dict)
    ):
        raise legacy.ValidationError("Disruptive technology expeditions must increase replenishment")
    for technology in ["tech.heartforge.tier_2", "tech.heartforge.tier_3", "tech.heartforge.tier_4", "tech.heartforge.tier_5"]:
        if technology not in technologies:
            raise legacy.ValidationError(f"Heartforge evolution is missing ecological consequence {technology}")
    if set(endgame) != {"protocol.severance", "protocol.containment"}:
        raise legacy.ValidationError("Both player-triggered endgame protocols need tier escalation effects")
...
def validate_no_wave_loop() -> None:
    ...
    forbidden = ["wave_number", "wave_timer", "next_wave", "wave_countdown"]
    combined = "\n".join((ROOT / path).read_text(encoding="utf-8").lower() for path in runtime_paths)
    for token in forbidden:
        if token in combined:
            raise legacy.ValidationError(f"Tier escalation must not introduce a recurring wave loop: {token}")


def validate_tier_documents_and_tests() -> None:
    ...
    for phrase in [
        "Saturation transfer",
        "Population suppression versus source suppression",
        "Tier 1 — Feral",
        "Tier 4 — Strategic",
        "Bounded spawn credit",
        "Autonomous suppression and anti-chore protection",
        "Acceptance criteria",
    ]:
    ...
    for token in [
        "_test_exact_saturation_transfer",
        "_test_population_headroom",
        "_test_recursive_transfer",
        "_test_physical_nests",
        "_test_progression_modifiers",
        "_test_physical_spawn_source",
        "_test_intelligence_progression",
        "_test_persistence",
    ]:
...
def main() -> int:
    validators = [
        ...
        validate_tier_runtime_contracts,
        validate_dynamic_event_modifiers,
        validate_no_wave_loop,
        validate_tier_documents_and_tests,
